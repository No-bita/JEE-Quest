...
import { X, Plus, Save, Image, FileText } from 'lucide-react';
import ImageUploader from './ImageUploader';
import MathRenderer from './MathRenderer';

// Enhanced Question type with image and latex support
interface Question {
  id: number;
  text: string;
  imageUrl?: string;
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="space-y-2 md:col-span-3">
                ...
                <div className="border rounded-md p-2">
                  <ImageUploader 
                    initialImage={currentQuestion?.imageUrl}
                    onImageSelected={(url) => handleQuestionChange('imageUrl', url)}
                  />
                </div>
              </div>
              ...
              {currentQuestion?.options.map(option => (
                <div key={option.id} className="p-4 border rounded-md">
                  ...
                  <div className="space-y-2 mt-2">
                    ...
                    <ImageUploader
                      initialImage={option.imageUrl}
                      onImageSelected={(url) => handleOptionChange(option.id, 'imageUrl', url)}
                      className="mt-2"
                    />
                  </div>
                </div>
              ...
              >
                {currentQuestion?.options.map(option => (
                  <div key={option.id} className="flex items-center space-x-2">
                    ...
                    <Label htmlFor={`answer-${option.id}`}>{option.id}</Label>
                  </div>
                ))}
              </RadioGroup>
            </div>
